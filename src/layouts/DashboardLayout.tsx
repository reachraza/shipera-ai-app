      ...
      >
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center ${isSidebarOpen ? 'px-5' : 'justify-center'} py-4 rounded-2xl font-bold transition-all duration-300 group ${
                  isActive 
        ...
        <div className="p-4 bg-muted/20 border-t border-border mt-auto">
          {isSidebarOpen && (
            <div className="flex items-center gap-4 px-4 py-4 mb-4 bg-card/50 rounded-2xl border border-border/50">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black truncate">{appUser?.email?.split('@')[0] || 'User'}</p>
          ...
          <div className="flex flex-col gap-2">
            <button className={`w-full flex items-center ${isSidebarOpen ? 'px-5 justify-start' : 'justify-center'} py-3 rounded-xl text-sm font-bold text-muted-foreground hover:bg-muted hover:text-foreground transition-all group`}>
